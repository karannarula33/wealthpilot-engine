 ...
 */
export function buildMorningBriefPrompt(marketContext) {
 ...
 */
export function buildPostSettleBriefPrompt(marketContext) {
    return buildAnalysisPrompt({
 ...
 */
export function buildEveningWrapPrompt(marketContext, todayRecommendations) {
    const recsContext = todayRecommendations.length > 0
        ? `Today's recommendations:\n${todayRecommendations.map((r) => `- ${r.symbol}: ${r.action} [${r.conviction}/5] "${r.summary}" — User: ${r.user_action || 'PENDING'}`).join('\n')}`
 ...
 */
export function buildIntradayAlertPrompt(symbol, triggerType, triggerDetails, marketContext, technicalData) {
    return buildAnalysisPrompt({
 ...
 */
export function buildUserQueryPrompt(query, marketContext) {
    return buildAnalysisPrompt({
 ...
 */
export function buildWeeklyDigestPrompt(marketContext, weekRecommendations, weekPerformance) {
    const recsContext = weekRecommendations.length > 0
        ? `This week's recommendations:\n${weekRecommendations.map((r) => `- ${r.symbol}: ${r.action} [${r.conviction}/5] — User: ${r.user_action}, Outcome: ${r.outcome_result || 'TBD'}`).join('\n')}`
