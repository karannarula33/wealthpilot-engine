...
import { formatQueryResponse, formatStatusMessage, formatHelpText, } from './templates';
...
export class WhatsAppService {
     ...
     */
    async sendMessage(userId, body, alertType, recommendationIds = []) {
        // Rate limit check
    ...
    async handleDetails(userId) {
        const { data: rec } = await db
            ...
            .single();
        if (!rec)
            return 'No recent recommendation to expand on.';
    ...
    async handleActed(userId) {
        const updated = await db.updateLatestRecommendation(userId, {
            user_action: 'ACTED',
        });
        if (!updated)
            return 'No pending recommendation to mark.';
    ...
    async handleSkip(userId) {
        ...
        if (!updated)
            return 'No pending recommendation to skip.';
    ...
    async handleQuery(userId, query) {
        ...
        const analysis = await this.ai.answerQuery(userId, query, portfolioCtx, marketCtx);
        return formatQueryResponse(analysis);
    }
    // --- Rate Limiting ---
    async canSend(userId, alertType) {
        const limits = {
        ...
        if (limit.cooldownMin > 0) {
            ...
            if (lastTime) {
                ...
                if (minutesSince < limit.cooldownMin)
                    return false;
            }
        }
        return true;
