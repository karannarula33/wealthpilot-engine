...
app.post('/api/webhook/whatsapp', async (req, res) => {
    try {
        ...
        const isValid = twilio.validateRequest(config.twilio.authToken, // This uses your config.ts setup
        twilioSignature, webhookUrl, req.body);
        if (!isValid) {
...
app.get('/api/market', async (req, res) => {
    try {
        const phase = req.query.phase || 'post-open';
        const data = await market.getMarketContext(phase);
    ...
    try {
        ...
        for (const h of holdings) {
            await db.from('holdings').upsert({
                ...
                unrealized_pnl_pct: h.avgCostPrice > 0
