...
import {
  ...
  buildPostSettleBriefPrompt,
  buildEveningWrapPrompt,
...
async function morningPrep(): Promise<void> {
...
async function postSettleBrief(): Promise<void> {
  const users = await db.getAllUsers();
...
async function eveningWrap(): Promise<void> {
  const users = await db.getAllUsers();
...
async function weeklyDigest(): Promise<void> {
  const users = await db.getAllUsers();
  const marketCtx = await market.getMarketContext('close');
...
async function syncPortfolios(): Promise<void> {
  const users = await db.getAllUsers();
  for (const user of users) {
    await syncUserPortfolio(user);
  }
...
async function resetQuietMode(): Promise<void> {
  const users = await db.getAllUsers();
  for (const user of users) {
...
async function syncUserPortfolio(user: any): Promise<void> {
  if (!user.zerodha_api_key) return;
 ...
 */
export async function runManualJob(jobName: string): Promise<void> {
